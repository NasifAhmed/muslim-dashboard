...
export default function useTime() {
  ...
  function getCurrentPrayerTime(currentTime: Date) {
    ...
    const timings = state.prayerTimingToday && state.prayerTimingToday.timings;
    ...
    type prayerTimeDataType = {
      ...
      BreakAfterSunset: string;
    };

    for (const prayerTime in prayerTimes) {
      const startMs = new Date(
        prayerTimes[prayerTime as keyof prayerTimeDataType].start as string,
      );
      // console.log("start time", startMs);
      const endMs = new Date(
        prayerTimes[prayerTime as keyof prayerTimeDataType].end as string,
      );
      ...
      if (currentTime >= startMs && currentTime <= endMs) {
        ...
        currentTime <= new Date(state.prayerTimingToday?.timings.Fajr as string)
      ) {
        return "Isha";
      }
    }
