...
const StateProvider = ({ children }: { children?: ReactNode }) => {
  ...
  function reducer(state: typeof initialState, action: stateAction): stateType {
    switch (action.type) {
      case "set location": {
        return {
          ...state,
      ...
      case "set timings": {
        ...
        window.localStorage.setItem("timings", JSON.stringify(jsonValue));
        return {
          ...state,
  ...
  useEffect(() => {
    ...
    if (state.prayerTimingsAll) {
      ...
      if (firstFajr.getMonth() !== dateToday.getMonth() && state.location) {
        getRawTimings(
